...
dice_image = dice_image.resize((20, 20), Image.Resampling.LANCZOS)
...
def create_labeled_input(frame, label_text, options, selected_var, randomize_command):
    tk.Label(frame, text=label_text, bg="#F7F6ED", fg="darkblue").pack(side=tk.LEFT)
...
def create_dropdown(parent, options, selected_var):
    dropdown = ttk.Combobox(parent, values=options, textvariable=selected_var)
...
tk.Label(frame, text="Name:", bg="#F7F6ED", fg="darkblue").pack(side=tk.LEFT)
name_entry = tk.Entry(frame, textvariable=selected_name)
...
create_labeled_input(frame2, "Gender:", genders, selected_gender, randomize_gender)
create_labeled_input(frame2, "Game Edition:", game_editions, selected_edition, randomize_edition)
...
# Function to finalize character and save data
...
# Add finalize button
finalize_button = tk.Button(root, text="Finalize", command=finalize_character, bg="green", fg="white")
